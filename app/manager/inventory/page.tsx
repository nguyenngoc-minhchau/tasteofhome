                      ...
                      <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex flex-col gap-2 ml-6">
                            ...
                            <Dialog open={historyProduct?.id === item.id && !!historyProduct} onOpenChange={(open) => !open && setHistoryProduct(null)}>
                              <DialogContent className="max-w-lg">
                                <DialogHeader>
                                  <DialogTitle>Adjustment History - {item.name}</DialogTitle>
                                </DialogHeader>
                                {item.adjustmentHistory && item.adjustmentHistory.length > 0 ? (
                                  <div className="space-y-2 max-h-80 overflow-y-auto">
                                    {(item.adjustmentHistory as any[]).map((adj, idx) => (
                                      <div key={idx} className="border rounded p-2 text-sm">
                                        <div><b>Date:</b> {adj.date}</div>
                                        <div><b>Type:</b> {adj.type}</div>
                                        <div><b>Quantity:</b> {adj.quantity}</div>
                                        <div><b>Reason:</b> {adj.reason}</div>
                                        <div><b>User:</b> {adj.user}</div>
                                        {typeof adj.notes === "string" && adj.notes.trim() !== "" && (
                                          <div><b>Notes:</b> {adj.notes}</div>
                                        )}
                                      </div>
                                    ))}
                                  </div>
                                ) : (
                                  <div className="text-muted-foreground text-center">No adjustment history.</div>
                                )}
                              </DialogContent>
                            </Dialog>
                          </div>
